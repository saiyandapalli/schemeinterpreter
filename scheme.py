...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first == 'define-macro':
        procedure = do_define_macro(rest, env)
        return procedure
    elif scheme_symbolp(first) and first in SPECIAL_FORMS:
        return SPECIAL_FORMS[first](rest, env)
    else:
        # BEGIN PROBLEM 5
        evaluated = scheme_eval(first, env)
        check_procedure(evaluated)
        if isinstance(evaluated, MacroProcedure):
            to_eval = MacroProcedure.apply_macro(evaluated, rest, env)
            return scheme_eval(to_eval, env)
        args = rest.map(lambda arg: scheme_eval(arg, env))
        return scheme_apply(evaluated, args, env)
        # END PROBLEM 5
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        new_frame_instance = self.env.make_child_frame(self.formals, args)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        expression = do_lambda_form(Pair(arguments, expressions.second), env)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        env.define(target.first, check_macro(Pair(target.second, expressions.second), env))
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def check_macro(expressions, env):
    check_form(expressions, 2)
    formals = expressions.first
    check_formals(formals)
    return MacroProcedure(formals, expressions.second, env)
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if scheme_symbolp(first) and first == 'define-macro':
                ...
                if isinstance(evaluated, MacroProcedure):
                    to_eval = MacroProcedure.apply_macro(evaluated, rest, env)
                    result = scheme_eval(to_eval, env)
                else:
                    args = rest.map(lambda arg: scheme_eval(arg, env))
                    result = scheme_apply(evaluated, args, env)
        return result
        # END
