...
(define (combine-halver x) (/ x 2))
(combine-halver (triple-add 6 8 10))
; expect 12

(lambda (y) (+ 1 (1/0)))
; expect (lambda (y) (+ 1 (1/0)))

(define incrementer (lambda (x) (+ 1 x)))
; expect incrementer
...
'(define incrementer (lambda (x) (+ 1 x)))
; expect (define incrementer (lambda (x) (+ 1 x)))

(quote (list 5 4 list))
; expect (list 5 4 list)
...
; #8
(begin (* 4 3) (begin (* 3 4) (begin (/ 12 0) (- 4 1))))
; expect Error: division by zero

(lambda (gtf) (begin 6 2 1 (* 8 7)))
; expect (lambda (gtf) (begin 6 2 1 (* 8 7)))

(begin (define x 7) (define y 92) (- x y))
; expect -85

(begin (define x '(3 2)) (define y '(4 2)) (cons x y))
; expect ((3 2) 4 2)

; #9
(lambda (x y z) (define a 27) (/ a (+ x y z)))
; expect (lambda (x y z) (define a 27) (/ a (+ x y z)))

(lambda (x) (lambda (y)))
; expect (lambda (x) (lambda (y)))

; #10
(define f (lambda (x y z) (define a 27) (/ a (+ x y z))))
; f

(define (/ 1 0) (lambda (x) (define v 2) (/ a (+ z))))
; expect Error
...
(- (let ((x 3)) (+ five (* x 2))) five)
; expect 6


(define t 7)
(let ((t 4) (y t)) (list t y))
;expect (4 7)

(let ((x 6)) (let ((x 9) (r x)) r))
; expect 6
...
(define z 2)
(define x 1)
(define rekt (mu (h) (list h z x)))
(define (yup x z p) (list (rekt p)))

(yup 7 8 0)
;expect ((0 8 7))

(define (tail combiner start n term)
(define (acc combiner start n term so-far)
  (if (= n 0)
      so-far
      (acc combiner (+ start 1) (- n 1) term (combiner so-far (term start)))
  )
)
...
(define (square x) (* x x))
(tail * 1 105 square)
;expect 
...
(list-of (+ x x) for x in '(4 5 6 7) if (odd? x))
;expect (10 14)
